...
_SYSTEM = (
    ...
    "- seller_accepted: TRUE if the seller agreed to my_last_offer. Judge this from "
    "CONTEXT, not keywords. A seller AGREES when, AFTER I name a specific price, they "
    "respond about logistics/timing/pickup/address (e.g. 'tomorrow?', 'come get it', "
    "'what time?', 'yes sir', 'I'm here', sharing an address) WITHOUT countering a "
    "different number — coordinating a meetup IN RESPONSE TO a price on the table = "
    "ACCEPTANCE, not rejection. But the SAME words are innocent when NO price is on "
    "the table: 'can I see it tomorrow?' / 'tomorrow?' used to schedule a VIEWING "
    "before any number is proposed is NOT acceptance — there is no deal yet. If the "
    "seller is non-committal ('let me think', 'I'll let you know', 'maybe') or counters "
    "a different number, it is NOT acceptance. When it's genuinely ambiguous, do NOT "
    "assume a deal.\n"
